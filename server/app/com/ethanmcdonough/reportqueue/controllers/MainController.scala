...
import java.io.{ IOException, File, InputStream, FileInputStream, InputStreamReader }

import models.{ User, DataInserter, UserReporter, ProgramReporter }
...
import scalaj.http.{ Token, HttpResponse, Http }
...
class HttpContextImpl @Inject() (system: ActorSystem) extends CustomExecutionContext(system, "http-context") with HttpContext

class MainController @Inject() (context: ExecutionContext, httpContext: HttpContextImpl, cc: ControllerComponents, env: Environment, config: Configuration) extends AbstractController(cc) {
  private def buildSimpleQuery(params: Tuple2[String, String]*): String = params.map { e => s"${URLEncoder.encode(e._1, "UTF-8")}=${URLEncoder.encode(e._2, "UTF-8")}" }.mkString("&")
  ...
  private val userInserter: Option[DataInserter] = config.getOptional[String]("reportqueue.google.sheets.users").map(DataInserter(_, dataStore, clientSecret, httpTransport, UserReporter))
  private val programInserter: Option[DataInserter] = config.getOptional[String]("reportqueue.google.sheets.programs").map(DataInserter(_, dataStore, clientSecret, httpTransport, ProgramReporter))
    ...
    request =>
      (for {
        ...
        consumer.map(Http("https://www.khanacademy.org/api/auth2/request_token")
          .postForm(Seq("oauth_callback" -> routes.MainController.oauthCallback.absoluteURL(request.secure)(request)))
  ...
  def report(reportType: String) = Action.async {
    request =>
      request.body.asJson.flatMap(body => (for {
        ...
        if (request.body.asJson.filter(b => (b \ "id").asOpt[String].isDefined && (b \ "reason").asOpt[String].isDefined).isEmpty) BadRequest(Json.toJson(Map("message" -> "Bad request")))
        else if ((inserters get reportType).isEmpty) NotFound(Json.toJson(Map("message" -> "Not found")))
        else if ((for {
          kaid <- request.session.get("kaid")
          name <- request.session.get("name")
