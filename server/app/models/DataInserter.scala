...
class DataInserter(spreadsheetId: String, dataStore: File, clientSecret: File, httpTransport: HttpTransport, reporter: Reporter) {
  ...
  def credentials: Credential = using[FileInputStream, Credential](new FileInputStream(clientSecret)) { clientSecret =>
    ...
    val flow: GoogleAuthorizationCodeFlow = new GoogleAuthorizationCodeFlow.Builder(httpTransport, jackson,
      googleClientSecrets, asJavaCollection(scopes)).setDataStoreFactory(googleDataStoreFactory)
  ...
  def insert(service: Sheets, user: User, id: String, date: Date, reason: String, ip: String): Either[String, String] = {
    ...
    if (alreadySubmitted(data, user, id))
      Left("You already reported that")
    else if (reason.length > userReasonCharCount)
      Left(s"Reasons can't exceed $userReasonCharCount characters in length")
    else if (quotaExceeded(data, user, date))
      Left(s"You can't submit more than $userReportTimeCount reports in ${userReportTimeMillis / 60000L} minutes")
    else {
      reporter.transform(id) match {
        case None => Left("Invalid ID")
        case Some(itemId) =>
          insertItems(service, itemId, user, reason, ip, date)
  ...
  @throws[IOException]
  def currentEntries(service: Sheets): List[List[Object]] = asScalaBuffer(service.spreadsheets.values
    .get(spreadsheetId, "A:E").execute.getValues).toList.map(asScalaBuffer(_).toList).drop(1)

  def quotaExceeded(data: List[List[Object]], user: User, date: Date): Boolean = data.count(e => Try(e(1).toString ==
    user.kaid && date.getTime - datef.parse(e(4).toString).getTime < userReportTimeMillis) getOrElse {
    Logger.error("Failed to process row: " + e)
    false
  }) >= userReportTimeCount

  def alreadySubmitted(data: List[List[Object]], user: User, id: String): Boolean = data.exists(e => (for {
    sheetReportedId <- (e lift 0).map(_.toString)
    userKaid <- (e lift 1).map(_.toString)
  } yield reporter.idsAreSame(id, sheetReportedId) && userKaid == user.kaid) getOrElse false)

  def insertItems(service: Sheets, id: String, user: User, reason: String, ip: String, date: Date): Unit = {
    val insertionBody: ValueRange = (new ValueRange).setValues(asList(asList(id.asInstanceOf[Object],
      user.kaid.asInstanceOf[Object], reason.asInstanceOf[Object], ip.asInstanceOf[Object], datef.format(date)
        .asInstanceOf[Object])))
