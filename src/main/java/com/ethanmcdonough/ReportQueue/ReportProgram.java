...
public class ReportProgram extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
	    	try {
	    		...
	    		try {
	        		InputStream configIS = new FileInputStream(new File(getServletContext().getRealPath("/WEB-INF/") + "config.json"));
			...
			String kaid = null, reason = null, programId = null;
	    		...
	    		try {
	    			...
	    			if(returnedProgContent == null) { 
	    				issuesWithKAAPI(request, response, "could not get https://www.khanacademy.org/api/labs/scratchpads/" + programId + " input stream"); 
	    				return; 
	    			}
	    		}
	    		...
	    		try {
	    			...
	    			issuesWithKAAPI(request, response, "could not parse JSON returned from KA API https://www.khanacademy.org/api/labs/scratchpads/" + programId); 
	    			return; 
	    		}
	    		...
	    		try {
	    			...
	    			issuesWithKAAPI(request, response, "Unable to parse https://www.khanacademy.org/api/internal/user/profile?kaid=" + kaid);
	    			log.log(Level.WARNING, "Unable to parse https://www.khanacademy.org/api/internal/user/profile?kaid=" + kaid, e);
	    			return;
	    		} finally {
	    			profileIS.close();
	    		}
	    		...
	    		try {
	        		clientSecret = new FileInputStream(new File(getServletContext().getRealPath("/WEB-INF/") + "client_secret.json"));
	        } catch (Exception e) {
	    		...
	    		GoogleClientSecrets googleClientSecrets = GoogleClientSecrets.load(googleJsonFactory, new InputStreamReader(clientSecret)); 
	    		
	    		clientSecret.close();
	    		
	    		log.info(credentials.getAbsolutePath());
	    		
	    		GoogleAuthorizationCodeFlow flow = new GoogleAuthorizationCodeFlow.Builder(googleHttpTransport, googleJsonFactory, googleClientSecrets, googleScopes)
	    		...
	    		ValueRange currentData = service.spreadsheets().values()
	    				.get(config.getString("spreadsheetId"), "A:E")
	    				.execute();
	    		...
	    		if(cdata.stream().filter(e -> 
	    			e.get(0) != null && 
	    			((String)e.get(0)).split("/")[5] != null && 
	    			((String)e.get(0)).split("/")[5].equals(finProgId) &&
	    			e.get(1) != null && ((String)e.get(1)).equals(finKaid)).count() > 0) {
	    		...
	    		if(cdata.stream().filter(e -> {
						try {
							...
							return e.get(1) != null && e.get(1).equals(finKaid) &&
							e.get(4) != null && reportedDate != null &&
	    		...
	    		ValueRange insertionBody = new ValueRange().setValues(Arrays.asList(
	    			Arrays.asList(
	    				(Object)progData.getString("url"),
	    				(Object)kaid,
	    				(Object)reason,
	    				(Object)ip,
	    				(Object)datef.format(insertDate)
	    			)
	    		));
	    		...
	    		response.setStatus(HttpServletResponse.SC_OK);
	    		writer.println(jsonFactory.createObjectBuilder()
	    				.add("success", true)
	    				.add("message", "Thanks for reporting, a Guardian will review this program.")
	    				.build().toString());
	    	} catch(Exception e) {
	    		internalServerError(request, response);
    ...
    private void respondError(String msg, int code, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    		PrintWriter writer = response.getWriter();
    ...
    private void internalServerError(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		respondError("Internal server error", HttpServletResponse.SC_INTERNAL_SERVER_ERROR, request, response);
    }

    private void unauthorized(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		respondError("Unauthorized", HttpServletResponse.SC_UNAUTHORIZED, request, response);
    }
    
    private void forbidden(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		respondError("Forbidden", HttpServletResponse.SC_FORBIDDEN, request, response);
    }
    
    private void issuesWithKAAPI(HttpServletRequest request, HttpServletResponse response, String note) throws ServletException, IOException {
		respondError("We're having some issues with the KA API right now (" + note + ")", HttpServletResponse.SC_INTERNAL_SERVER_ERROR, request, response);
    }
    ...
    private void reasonTooLong(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		respondError("The reason may not be more than " + userReasonCharCount + " characters", HttpServletResponse.SC_BAD_REQUEST, request, response);
    }
    
    private void alreadyReported(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		respondError("You already reported that program", HttpServletResponse.SC_BAD_REQUEST, request, response);
    }
    
    private void tooManyReports(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		respondError("You cannot report more than " + userReportTimeCount + " programs per hour.", HttpServletResponse.SC_BAD_REQUEST, request, response);
    }
    
    private void invalidProgramId(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    		respondError("Invalid program ID", HttpServletResponse.SC_BAD_REQUEST, request, response);
    }
    
    private void invalidKaid(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		respondError("Invalid kaid", HttpServletResponse.SC_BAD_REQUEST, request, response);
    }
    
	private void methodNotAllowed(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		respondError("Method not allowed", HttpServletResponse.SC_METHOD_NOT_ALLOWED, request, response);
	}
	
	private void ratelimit(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		respondError("You're sending too many requests.  Please wait a few minutes before trying again.", 429, request, response);
	}
}